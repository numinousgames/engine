// vec2.h
#ifndef NGE_MATH_VEC2_H
#define NGE_MATH_VEC2_H
 ...
 */
template <typename T>
TVec2<T> operator<<( const TVec2<T>& v, uint32 shift );
 ...
 */
template <typename T>
TVec2<T> operator>>( const TVec2<T>& u, uint32 shift );
 ...
 */
template <typename T, typename U>
bool operator==( const TVec2<T>& u, const TRef2<U>& v );
 ...
 */
template <typename T, typename U>
bool operator==( const TRef2<T>& u, const TRef2<U>& v );
...
template <typename T>
inline
const T& TVec2<T>::operator[]( uint32 index ) const
{
...
template <typename T>
inline
T& TVec2<T>::operator[]( uint32 index )
{
    assert( index >= 0 && index < 2 );
    return ( &x )[index];
}
